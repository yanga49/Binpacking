...
def main():
    ...
    offline_binpacker = [NextFitOffline(), FirstFitDecreasing(),
                         BestFitDecreasing(),
                         MostTerribleDecreasing(),
                         WorstFitDecreasing(),
                         GreedyHeuristic(optimal),
                         RefinedFirstFitDecreasing()]
    case = [cases[0]]
    run_bench_time(offline_binpacker, case, False)
    # plot histogram:
    # linear = ['greedy', 'most terrible', 'next fit']
    # nonlinear = ['first fit', 'best fit', 'worst fit',
    #       'refined first fit', 'python-binpacking']
    # all_names = linear + nonlinear
    # for algo in all_names:
    #     val = load_bench_measurements(algo, case,
    #           'outputs/pyperf_measurements_offline_case.json')
    #     vals.append(val)
...
def run_bench_time(algorithms: list, cases: list[Case],
                   is_online: bool) -> None:
    ...
    for algo in algorithms:
        for case in cases:
            name = run_algorithm(algo, case, is_online)['name'] + "_" +\
                                 basename(case)
            if is_online:
...
def load_bench_measurements(algo: str, cases:
                            list[Case], json_filename: str) -> (list, str):
    ...
    for case in cases:
        ...
        # compile all values using one algorithm to a list
...
def plot_hist(vals: list, dataset: str, classify: str) -> None:
    ...
    plt.title(f"Execution time for {classify} bin packing \
        algorithms on binpp dataset {dataset}")
